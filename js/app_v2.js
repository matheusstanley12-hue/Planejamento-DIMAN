...
window.Router = (() => {
  ...
  async function navigate(name, params = {}) {
    ...
    const isSameRoute = (current === name);
    ...
    current = name;
    localStorage.setItem('diman_last_route', JSON.stringify({ name, params: currentParams }));
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    // Navigate to home or worker panel, or restore last route
    let defaultRoute = session.perfil === 'Executante' ? 'worker-panel' : 'home';
    let defaultParams = {};
    try {
      const saved = localStorage.getItem('diman_last_route');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed && parsed.name) {
          // If Executante, ensure they don't escape to restricted areas
          if (session.perfil === 'Executante' && !['worker-panel', 'qrview'].includes(parsed.name)) {
            // override
          } else {
            defaultRoute = parsed.name;
            defaultParams = parsed.params || {};
          }
        }
      }
    } catch(e) {}
    ...
    <div style="position:relative;width:100%;max-width:420px;padding:var(--space-4);animation:fadeInUp 0.6s ease forwards;">
      <div class="card" style="padding:var(--space-8);border-color:var(--border-hover);box-shadow:var(--shadow-xl),0 0 60px rgba(21,101,192,0.15);">
        ...
        <div style="text-align:center;margin-bottom:var(--space-6)">
          <div style="display:inline-flex;align-items:center;justify-content:center;margin-bottom:var(--space-2);width:100%;">
            <img src="logo.png?v=${window.CACHE_VERSION || Date.now()}" style="width:100%;max-width:340px;height:auto;max-height:220px;object-fit:contain;" alt="Logo GEOSOL" />
          </div>
...
function renderShell(session) {
  ...
  const navItems = [
    ...
    { route:'workforce',  label:'Mão de Obra',           icon:'users',          perm:'workforce',   section:'RECURSOS HUMANOS' },
  ...
  if (typeof ActionPlanModule !== 'undefined') Router.register('action-plans', () => ActionPlanModule.render());
