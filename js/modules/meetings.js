...
window.MeetingsModule = (() => {
  ...
  function getInitialMeetingDate() {
    const tasks = window.DB && window.DB.meetingTasks ? window.DB.meetingTasks.getAll() : [];
    if (tasks.length > 0) {
        ...
        <header class="page-header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
          ...
          <div style="display:flex; gap:10px; align-items:center;">
            <div style="display:flex; align-items:center; gap:8px; background:var(--bg-card); padding:6px 12px; border-radius:6px; border:1px solid var(--border-default);">
              <label for="meeting-date-select" style="font-weight:bold; font-size:14px; color:var(--text-secondary); margin:0;">Reunião:</label>
              <input type="date" id="meeting-date-select" class="form-control" style="width:auto; font-weight:bold; border:none; background:transparent; padding:0; outline:none; font-family:inherit;" value="${selectedMeetingDate}" onchange="MeetingsModule.onDateChange()">
            </div>
              ...
              return `
                <tr class="task-row" style="border-bottom:1px solid var(--border-card); cursor:pointer; transition:background 0.2s; ${isDone ? 'opacity:0.7; background:rgba(0,0,0,0.02);' : ''}" onclick="MeetingsModule.openNewTaskModal('${t.id}')" onmouseover="this.style.background='var(--bg-hover, rgba(0,0,0,0.02))'" onmouseout="this.style.background='${isDone ? 'rgba(0,0,0,0.02)' : 'transparent'}'">
                  ...
                  <td style="padding:var(--space-3); vertical-align:top;">
                    ...
                    ${(t.local || t.client) ? `
                      <div style="display:flex; gap:8px; margin-top:8px; margin-bottom:4px; flex-wrap:wrap;">
                        ${t.client ? `<span style="font-size:11px; font-weight:600; background:var(--color-purple-bg, rgba(206, 147, 216, 0.12)); color:var(--color-purple, #CE93D8); padding:4px 8px; border-radius:var(--radius-full); display:inline-flex; align-items:center;"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="width:14px; height:14px; margin-right:4px;"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>${t.client}</span>` : ''}
                        ${t.local ? `<span style="font-size:11px; font-weight:600; background:var(--color-orange-bg, rgba(255, 167, 38, 0.12)); color:var(--color-orange, #FFA726); padding:4px 8px; border-radius:var(--radius-full); display:inline-flex; align-items:center;"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="width:14px; height:14px; margin-right:4px;"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" /></svg>${t.local}</span>` : ''}
                      </div>
                    ` : ''}
                    ${t.comments ? `
  ...
  function downloadPDF() {
    ...
    const rows = tasks.map(t => {
      let descWithTags = t.description;
      let tags = [];
      if (t.tag) tags.push(t.tag);
      if (t.client) tags.push(t.client);
      if (t.local) tags.push(t.local);
      if (tags.length > 0) {
        descWithTags = `[${tags.join(' | ')}]\n${t.description}`;
      }
