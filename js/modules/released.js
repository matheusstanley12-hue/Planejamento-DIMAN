window.ReleasedModule = (() => {
                      ...
                      <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                        ...
                        <div style="display:flex; align-items:center; gap:8px;">
                          ...
                          ${canEdit ? `
                            ...
                            <button class="btn-premium-edit" 
                                    style="color: var(--color-warning); background: rgba(245, 158, 11, 0.1);"
                                    title="Voltar para Manutenção"
                                    onclick="event.stopPropagation(); window.ReleasedModule.revertToMaintenance('${e.id}')">
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
  ...
  function revertToMaintenance(id) {
    if (window.uiConfirm) {
        ...
        if (confirm('Tem certeza que deseja retornar este equipamento para o status Em Manutenção?')) doRevert(id);
    }
  }

  function doRevert(id) {
    const eq = window.DB.equipment.list().find(e => e.id === id);
    if (!eq) return;
    
    // Create an audit log entry
    if (window.Auth && window.Auth.addAuditLog) {
        window.Auth.addAuditLog('REVERT_MAINTENANCE', `Equipamento ${eq.codigo} retornado de Liberado para Em Manutenção`, eq.id);
    }
    
    window.DB.equipment.update(id, { 
        status: 'Em Manutenção', 
        dataRealLiberacao: null,
        dataLiberacaoAtual: null,
        dataLiberacaoReal: null
    });
    
    if (window.Toast) window.Toast.success('Equipamento Retornado', 'O equipamento voltou para o quadro de manutenção.');
    if (window.Router) window.Router.navigate('released', { force: true });
  }

  return { render, filter, revertToMaintenance };
})();
