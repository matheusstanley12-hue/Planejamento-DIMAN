window.EquipmentPanel = (() => {
      ...
      <div style="max-width:1200px;margin:0 auto;padding:var(--space-6);">
        ...
        <div style="margin-bottom:var(--space-4);display:flex;align-items:center;justify-content:space-between;gap:var(--space-3);">
          ...
          <div style="display:flex;gap:var(--space-2);">
            ${window.Auth && Auth.getCurrentUser()?.perfil === 'Administrador' ? `
              <button class="btn btn-ghost btn-sm" onclick="EquipmentPanel.deleteEquipment()" style="color:var(--color-danger);border:1px solid transparent;" title="Excluir Equipamento permanentemente" onmouseover="this.style.borderColor='var(--color-danger)'" onmouseout="this.style.borderColor='transparent'">
  ...
  function deleteEquipment() {
    ...
    if (confirm(`ATENÇÃO! Tem certeza que deseja excluir completamente o equipamento ${eq.codigo} e TODAS as suas tarefas, peças e histórico? Esta ação não pode ser desfeita.`)) {
      ...
      const eqTasks = DB.tasks.getByEquipment(currentEqId);
      eqTasks.forEach(t => DB.tasks.delete(t.id));
      
      // Remover todas as pecas atreladas
      const eqParts = DB.parts.list(currentEqId);
      eqParts.forEach(p => DB.parts.delete(p.id));
      
      // Finalmente remove o equipamento
      DB.equipment.delete(currentEqId);
      
      Toast.success('Equipamento Excluído', `O equipamento ${eq.codigo} foi removido com sucesso.`);
