...
const TasksModule = (() => {
  ...
  function render() {
    ...
    const grouped = {};
    tasks.forEach(t => {
      if (!grouped[t.equipmentId]) grouped[t.equipmentId] = [];
      grouped[t.equipmentId].push(t);
    });
    ...
    return `<div class="page-container">
      <div class="section-header">
        <div class="section-title"><div class="section-title-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="white"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg></div>Gestão de Tarefas</div>
        <button class="btn btn-primary" onclick="TasksModule.openCreate()">+ Nova Tarefa</button>
      </div>
      <div class="filter-row" style="margin-bottom:var(--space-5);">
        <select onchange="TasksModule.setEq(this.value)" style="width:200px;">
          <option value="">Todos os Equipamentos</option>
          ${eqs.map(e=>`<option value="${e.id}" ${eqFilter===e.id?'selected':''}>${e.codigo}</option>`).join('')}
        </select>
        <span style="color:var(--text-muted);font-size:var(--text-sm)">${tasks.length} tarefas</span>
        <span class="badge badge-success">${tasks.filter(t=>t.status==='Concluída').length} concluídas</span>
        <span class="badge badge-primary">${tasks.filter(t=>t.status==='Em Andamento').length} em andamento</span>
        <span class="badge badge-danger">${tasks.filter(t=>t.status==='Bloqueada').length} bloqueadas</span>
        <span class="badge badge-danger">${tasks.filter(t=>t.critico).length} críticas</span>
      </div>
      <div style="display:flex;flex-direction:column;gap:var(--space-2);" class="stagger">
        ${tasks.length===0?'<div class="empty-state"><p>Nenhuma tarefa encontrada</p></div>':''}
        ${Object.keys(grouped).map(eqId => {
          const eq = equipMap[eqId];
          const eqTasks = grouped[eqId];
          const pct = eq ? (eq.pctAvanco || 0) : 0;
          return `
            <div style="background:var(--bg-card);border:1px solid var(--border-card);border-radius:var(--radius-md);overflow:hidden;margin-bottom:var(--space-2);">
              <div style="padding:var(--space-3) var(--space-4);background:var(--bg-base);cursor:pointer;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--border-card);" onclick="const body = this.nextElementSibling; const icon = this.querySelector('.expand-icon'); if(body.style.display==='none'){body.style.display='block';icon.style.transform='rotate(90deg)';}else{body.style.display='none';icon.style.transform='rotate(0deg)';}">
                <div style="display:flex;align-items:center;gap:var(--space-3);">
                  <span class="expand-icon" style="transition:transform .2s;display:inline-block;color:var(--text-muted);">▶</span>
                  <div style="font-weight:800;font-size:1.1rem;color:var(--brand-primary-light)">${eq ? eq.codigo : 'Equipamento Desconhecido'}</div>
                  <div style="font-size:var(--text-sm);color:var(--text-muted)">${eq ? eq.nome : ''}</div>
                </div>
                <div style="display:flex;align-items:center;gap:var(--space-4);">
                  <span class="badge badge-ghost">${eqTasks.length} tarefas</span>
                  <div style="width:100px;">
                    <div class="progress-track"><div class="progress-fill ${pct>=80?'success':pct>=50?'':'warning'}" style="width:${pct}%"></div></div>
                    <div style="font-size:10px;text-align:right;color:var(--text-muted)">${pct}%</div>
                  </div>
                </div>
              </div>
              <div style="display:none;padding:var(--space-3);background:var(--bg-card);">
                <div style="display:flex;flex-direction:column;gap:var(--space-2);">
                  ${eqTasks.map(t => renderTaskRow(t, equipMap)).join('')}
                </div>
              </div>
            </div>
          `;
  ...
  function taskForm(t) {
    ...
    return `<div style="display:flex;flex-direction:column;gap:var(--space-4);">
      <div class="form-row"><div class="form-group"><label>Equipamento *</label><select id="tk-eq">${eqs.map(e=>`<option value="${e.id}" ${t?.equipmentId===e.id?'selected':''}>${e.codigo}</option>`).join('')}</select></div>
      ...
      <div class="form-row"><div class="form-group"><label>Disciplina</label><select id="tk-disc">${discs.map(d=>`<option ${t?.disciplina===d?'selected':''}>${d}</option>`).join('')}</select></div>
  ...
  return { render, openCreate, openEdit, save, deleteTask, setEq };
})();
